...
import Environment.Env (ENV (playpen, playpenTaken), kids, rows, columns)
...
cells2Corners (x:xs) answ dist n m =
    let
        distance = calculateDistance x (0,0) (0,m-1) (0,n-1) (m-1,m-1)
    in if distance < dist
       then cells2Corners xs x distance n m
...
calculateDistance :: Integral a => (a, a) -> (a, a) -> (a, a) -> (a, a) -> (a, a) -> a
